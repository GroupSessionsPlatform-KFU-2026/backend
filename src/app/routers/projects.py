...
async def get_project_tags(
    project_id: UUID,
    project_service: ProjectServiceDep,
) -> PaginatedResponse[ProjectTagPublic]:
    project_tags = await project_service.get_project_tags(project_id)
    total = await project_service.count_project_tags(project_id)

    return build_paginated_response(
        items=list(project_tags),
        total=total,
        offset=0,
        limit=100,
    )
